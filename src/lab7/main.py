...
from primitives import SceneObject
...
def app():
    ...
    scene_object = SceneObject(main_object)
    rendered_object = render_object(scene_object, renders[current_render], window_info)
    ...
    while running:
        ...
        if button(screen, font, transform_buttons[0], "Перенос") and button_clicked:
            try:
                ...
                # Домножаем ее на текущую матрицу трансформации объекта
                scene_object.transform = np.dot(trans_matrix, scene_object.transform)
            except ValueError:
                pass
            button_clicked = False
        ...
        if button(screen, font, transform_buttons[1], "Масштаб") and button_clicked:
            try:
                ...
                # Получаем центр ИСХОДНОЙ модели
                center = scene_object.model.get_center()
                # Создаем матрицу масштабирования относительно центра
                t1 = translation_matrix(-center.x, -center.y, -center.z)
                s = scale_matrix(sx, sy, sz)
                t2 = translation_matrix(center.x, center.y, center.z)
                scale_mtx = np.dot(t2, np.dot(s, t1))
                # Домножаем на нее основную матрицу
                scene_object.transform = np.dot(scale_mtx, scene_object.transform)
            except ValueError:
                pass
            button_clicked = False
        ...
        if button(screen, font, transform_buttons[2], "Поворот X") and button_clicked:
            try:
                ...
                center = scene_object.model.get_center()
                # Создаем матрицу вращения относительно центра
                t1 = translation_matrix(-center.x, -center.y, -center.z)
                r = rotation_x_matrix(angle)
                t2 = translation_matrix(center.x, center.y, center.z)
                rot_mtx = np.dot(t2, np.dot(r, t1))
                scene_object.transform = np.dot(rot_mtx, scene_object.transform)
            except ValueError:
                pass
            button_clicked = False
        ...
        def apply_reflection(reflection_matrix_func):
            """Вспомогательная функция для отражений относительно центра."""
            center = scene_object.model.get_center()
            t1 = translation_matrix(-center.x, -center.y, -center.z)
            reflect_m = reflection_matrix_func()
            t2 = translation_matrix(center.x, center.y, center.z)
            final_matrix = np.dot(t2, np.dot(reflect_m, t1))
            scene_object.transform = np.dot(final_matrix, scene_object.transform)

        if button(screen, font, transform_buttons[5], "Отражение XY") and button_clicked:
            apply_reflection(reflection_xy_matrix)
            button_clicked = False

        if button(screen, font, transform_buttons[6], "Отражение XZ") and button_clicked:
            apply_reflection(reflection_xz_matrix)
            button_clicked = False

        if button(screen, font, transform_buttons[7], "Отражение YZ") and button_clicked:
            apply_reflection(reflection_yz_matrix)
            button_clicked = False
        ...
        if button(screen, font, transform_buttons[8], "Поворот вокруг прямой") and button_clicked:
            try:
                ...
                if len(p1_coords) == 3 and len(p2_coords) == 3:
                    ...
                    # Используем новую функцию для получения матрицы
                    rot_line_mtx = rotation_around_line_matrix(p1, p2, angle)
                    scene_object.transform = np.dot(rot_line_mtx, scene_object.transform)
            except ValueError:
                pass
            button_clicked = False

        if button(screen, font, transform_buttons[9], "Сброс") and button_clicked:
            # Просто сбрасываем матрицу в единичную
            scene_object.reset_transform()
            button_clicked = False
        ...
        if button(screen, font, file_buttons[0], "Загрузить OBJ") and button_clicked:
            ...
            try:
                ...
                # Нормализуем модель один раз после загрузки
                normalize_and_center_object(main_object, config.OBJ_TARGET_SIZE)
                # Создаем новый объект сцены с новой моделью
                scene_object = SceneObject(main_object)
            except Exception as e:
                print(f"Не удалось загрузить или обработать файл {filename}: {e}")
            button_clicked = False
        ...
        if button(screen, font, file_buttons[1], "Сохранить OBJ") and button_clicked:
            ...
            file_path = os.path.join(models_dir, filename)
            try:
                # Создаем временную копию объекта для сохранения
                object_to_save = Object(polies=[Polygon(p.vertices.copy()) for p in scene_object.model.polygons])
                # Применяем к КОПИИ текущую трансформацию
                object_to_save.apply_transformation(scene_object.transform)
                save_obj(object_to_save, file_path)
            except Exception as e:
                print(f"Ошибка сохранения: {e}")
            button_clicked = False

        # После любого действия, которое могло изменить состояние, перерисовываем
        rendered_object = render_object(scene_object, renders[current_render], window_info)
